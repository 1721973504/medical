		...
		<div class="panel admin-panel">
			...
			<div class="padding border-bottom">
				<ul class="search" style="padding-left:10px;">
					<li><a class="button border-main icon-plus-square-o"
						href="<%=path%>/system/UserServlet?m=input"> 添加用户</a></li>
					<li>搜索：</li>
			...
			<table class="table table-hover text-center">
				...
				<%
					...
					Object[] params = {};
					Page pageBean = userDao.queryOjectsByPage(sql, params, p,
							Constant.ROW);
				%>
				<%
				...
				<td><%=(i+1) %></td>
					...
					<td><div class="button-group">
							<a class="button border-main"
								href="<%=path%>/system/UserServlet?m=get&id=<%=model.getUserid()%>"><span
								class="icon-edit"></span> 修改</a> <a class="button border-red"
								href="<%=path%>/system/UserServlet?m=del&id=<%=model.getUserid()%>"><span
			...
			<div class="pagelist">
				...
				%>
				<a
					href="<%=path%>/system/UserServlet?m=list&pageNo=<%=pageBean.prePage()%>">上一页</a>
				<%
					} else {
				%>
				...
				%>
				<a
					href="<%=path%>/system/UserServlet?m=list&pageNo=<%=pageBean.lastPage()%>">下一页</a>
				<%
					} else {
				%>
				...
				%>
				<a
					href="<%=path%>/system/UserServlet?m=list&pageNo=<%=pageBean.getPages()%>">尾页</a>
			</div>
		</div>
	</form>
